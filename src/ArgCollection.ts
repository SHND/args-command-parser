...
class ArgCollection {
  ...
  get data(): Data {
    ...
    Object.values(this._data.shortSwitches).forEach(s => { shortSwitches[s.name] = s.values });
    const longSwitches : { [key: string]: string[] } = {}
    Object.values(this._data.longSwitches).forEach(s => { longSwitches[s.name] = s.values });
  ...
  private addShortSwitch(switchOrArgName: string | ShortSwitch, argValues: string[] = []): void {
    if (switchOrArgName instanceof ShortSwitch) {
      this._data.shortSwitches[switchOrArgName.name] = new ShortSwitch(switchOrArgName);
    } else if (typeof (switchOrArgName) === 'string') {
      this._data.shortSwitches[switchOrArgName] = new ShortSwitch(switchOrArgName, argValues);
    }
  }

  private addLongSwitch(longSwitch: LongSwitch): void
  private addLongSwitch(argName: string, argValues?: string[]): void
  private addLongSwitch(switchOrArgName: string | LongSwitch, argValues: string[] = []): void {
    if (switchOrArgName instanceof LongSwitch) {
      this._data.longSwitches[switchOrArgName.name] = new LongSwitch(switchOrArgName);
    } else if (typeof (switchOrArgName) === 'string') {
      this._data.longSwitches[switchOrArgName] = new LongSwitch(switchOrArgName, argValues);
    }
  }
